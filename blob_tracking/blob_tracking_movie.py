...
shotnr  = 1100803015
frame0 = 000                            # Begin analysis at this frame. 
nframes = 2000                          # Number of frames to analyze
...
#    plt.plot( z0, R0, 'ko' )
    ...
    if ( fwhm_r_idx.ndim == 3 ):
        print 'Width: radially: %3.1fcm poloidally: %3.1fcm' % ( blob_ell_avg[idx,0], blob_ell_avg[idx,1] )

    for t in tau:
        # Re-grid data and plot
        zi = griddata(rz_array.reshape(64*64, 2), frames[t0+t,:,:].reshape( 64*64 ), xyi.reshape( 64*64, 2 ), method='linear' )
    
        plt.figure()
        plt.title('frame %05d' % (t0 + t) )        
        plt.contour (xxi[0,:], yyi[:,0], zi.reshape(64, 64), 15, linewidths=0.5, colors='k')
        plt.contourf(xxi[0,:], yyi[:,0], zi.reshape(64, 64), 15, cmap = plt.cm.hot, levels=np.linspace(0.0,3.5,32))
        plt.colorbar(ticks=np.arange(0.0, 3.5, 0.5), format='%3.1f')
        F = plt.gcf()
 
        # Get indices for COM coordinates
        foo = np.round(xycom).astype('int')[t + tau_b]
        xcom = np.array([ rz_array[ foo[1], foo[0], 0], rz_array[ foo[1], foo[0], 1] ])    
        # Great, we can make error plots
        if ( fwhm_r_idx.ndim == 3 ):
            yerr_p = [xcom[1] - Rzi[fwhm_p_idx[0, 1, t+tau_b], fwhm_p_idx[0, 0, t+tau_b], 1] , Rzi[fwhm_p_idx[1, 1, t+tau_b], fwhm_p_idx[1, 0, t+tau_b], 1] - xcom[1] ]
            xerr_p = [xcom[0] - Rzi[fwhm_r_idx[0, 1, t+tau_b], fwhm_r_idx[0, 0, t+tau_b], 0] , Rzi[fwhm_p_idx[1, 1, t+tau_b], fwhm_p_idx[1, 0, t+tau_b], 0] - xcom[0] ]
        
            plt.errorbar( xcom[0], xcom[1], xerr=[np.ones(1)*xerr_p[0], np.ones(1)*xerr_p[1]], yerr=[np.ones(1) * yerr_p[0], np.ones(1)*yerr_p[1] ], fmt='--wo' )

        elif ( fwhm_r_idx.ndim == 2 ):
            plt.plot(xcom[0], xcom[1], 'wo')
        F.savefig('../../blob_tracking/%d/frames/%d_frame%05d.png' % (shotnr, shotnr, t0 + t) )
        plt.close()
        
        frame_list.append(t0+t)
...
np.savez('../../blob_tracking/%d/frames/frame_list.npz' % (shotnr), frame_list = frame_list)
